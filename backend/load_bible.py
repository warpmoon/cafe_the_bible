...
def load_bible():
    print("Loading books...")
    book_map = {}
    for abbr, name, testament, order in BIBLE_BOOKS:
        book, created = Book.objects.get_or_create(
            abbreviation=abbr,
            defaults={'name': name, 'testament': testament, 'order': order}
        )
        book_map[abbr] = book
    ...
    for ref, content in data.items():
        # ref example: "창1:1", "요10:10", "고전13:1"
        match = re.match(r'([^\d]+)(\d+):(\d+)', ref)
        if match:
            abbr, chapter, verse_num = match.groups()
            if abbr in book_map:
